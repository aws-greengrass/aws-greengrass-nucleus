...
public class DeploymentCloudServiceIntegTest {
    ...
    static void setIotClient() {
        ...
        deviceProvisioningHelper =
                new DeviceProvisioningHelper(Region.US_EAST_1, StaticCredentialsProvider.create(awsCreds));
        iotClient = deviceProvisioningHelper.getIotClient();
    }
    ...
    void cleanUp() {
        ...
        IotJobsUtils.cleanThingGroup(iotClient, thingGroupResp.thingGroupName());
        createdIotJobIdList.forEach(jobId -> IotJobsUtils.cleanJob(iotClient, jobId));
...
//    @Test
    void GIVEN_blank_kernel_WHEN_create_deployment_on_thing_group_THEN_new_services_deployed_and_job_is_successful() throws Exception {
        ...
        IotJobsUtils
                .waitForJobExecutionStatusToSatisfy(iotClient, jobId1, thingInfo.getThingName(), Duration.ofMinutes(5),
