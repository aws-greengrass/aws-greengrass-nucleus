...
public class PackageManagerIntegTest extends BaseITCase {
    ...
    void GIVEN_component_with_archived_artifact_WHEN_prepareArtifacts_THEN_unarchives_artifacts() throws Exception {
        ...
        when(mockServiceHelper.downloadPackageRecipe(any())).thenReturn(
                new PackageRecipe(RecipeTemplateVersion.JAN_25_2020, "A", new Semver("1.0.0"), "", "",
                        ...
                        Utils.immutableMap("all", Arrays.asList(
                                new ComponentArtifact(new URI("greengrass:zip.zip"), null, null,
                                        Unarchive.ZIP.name()))), Collections.emptyMap(), null));
        kernel.getContext().put(GreengrassPackageServiceHelper.class, mockServiceHelper);

        // THEN
        kernel.getContext().get(PackageManager.class).preparePackages(Arrays.asList(ident)).get(10, TimeUnit.SECONDS);
