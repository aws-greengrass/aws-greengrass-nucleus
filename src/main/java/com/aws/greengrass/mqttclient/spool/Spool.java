...
public class Spool {
    ...
    private CloudMessageSpool setupDiskSpooler(ExecutorService executorService)
        ...
        // Fetch the persisted ids and advance nextId synchronously, before spawning the background
        // load. Reading the id list is cheap (no per-message body reads); the slow part that motivated
        // issue #1832 is the per-message getMessageById() loop, which stays on the background thread.
        //
        // Advancing nextId here guarantees a later addMessage() always allocates an id above every id
        // still on disk, so it cannot collide with a persisted row. If getAllMessageIds() fails, the
        // IOException propagates to setupSpooler() and we fall back to InMemorySpool, matching the
        // pre-async behavior instead of retaining a disk spooler with nextId=0 over a populated DB.
        List<Long> diskQueueOfIds = new ArrayList<>();
        long highestId = -1;
        for (long id : persistenceSpool.getAllMessageIds()) {
            diskQueueOfIds.add(id);
            highestId = Math.max(highestId, id);
        }
        nextId.set(highestId + 1);

        // Run the per-message body load on a background thread to avoid blocking kernel startup
        // (issue #1832). popId() will wait for this to complete before draining messages.
        diskQueueLoaded = CompletableFuture.runAsync(() -> {
            try {
                persistentQueueSync(diskQueueOfIds, persistenceSpool);
     ...
     *
     * <p>This may be invoked on the MQTT connection-event (CRT event-loop) thread, so it must not
     * block. If the disk-to-memory load is still in progress, the prune is deferred to run on the
     * spool executor once the load completes, rather than waiting inline. If the load is already
     * done, the prune runs synchronously on the calling thread.</p>
     */
    public void popOutMessagesWithQosZero() {
        if (diskQueueLoaded.isDone()) {
            removeMessagesWithQosZero(false);
        } else {
            // Do not block the caller (potentially the CRT event-loop thread) on the disk load.
            // Defer pruning until the load finishes, on the managed executor.
            diskQueueLoaded.thenRunAsync(() -> removeMessagesWithQosZero(false), executorService)
                    .whenComplete((unused, t) -> {
                        if (t != null) {
                            logger.atWarn().cause(t)
                                    .log("Deferred QoS 0 offline prune failed after disk queue load");
                        }
                    });
        }
    }
    ...
    private void removeMessagesWithQosZero(boolean needToCheckCurSpoolerSize) {
        ...
        // Intentionally done OUTSIDE the lock so waiting can never block threads holding it.
        try {
            awaitDiskQueueLoaded();
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
        ...
        // Hold the lock so the deque iteration and size accounting stay consistent with concurrent addMessage calls.
        try (LockScope ls = LockScope.lock(lock)) {
    ...
    /**
     * Load persisted message bodies from the persistenceSpool plugin's disk database into
     * queueOfMessageId. Runs on a background thread to avoid blocking kernel startup (issue #1832).
     *
     * <p>Does not set {@link #nextId}: it is advanced synchronously in {@link #setupDiskSpooler}
     * from the full disk id list, before this method runs, so a later addMessage() cannot collide
     * with a persisted id even if this load stops early (e.g. the persisted queue exceeds the
     * configured spool size) or fails partway through.</p>
     *
    ...
    public void persistentQueueSync(Iterable<Long> diskQueueOfIds, CloudMessageSpool persistenceSpool)
        ...
        // load message bodies into the runtime queue up to capacity. nextId has already been
        // advanced past every persisted id by the caller, so this method only appends.
        int numMessages = 0;
