...
public interface UserEnv extends StdCallLibrary {
     ...
     * @param profile Pointer to a PROFILEINFO structure. LoadUserProfile fails and returns ERROR_INVALID_PARAMETER if
     *        the dwSize member of the structure is not set to sizeof(PROFILEINFO) or if the lpUserName member is NULL.
     ...
     * @param hToken Token for the user, returned from the LogonUser function. If this is a primary token, the token
     *        must have TOKEN_QUERY and TOKEN_DUPLICATE access. If the token is an impersonation token, it must have
     *        TOKEN_QUERY access. For more information, see Access Rights for Access-Token Objects. If this parameter is
     *        NULL, the returned environment block contains system variables only.
     * @param bInherit Specifies whether to inherit from the current process' environment. If this value is TRUE, the
     *        process inherits the current process' environment. If this value is FALSE, the process does not inherit
     ...
     * @return <code>true</code> if successful; otherwise <code>false</code>
     * @see <a href=
     *      "https://docs.microsoft.com/en-us/windows/win32/api/userenv/nf-userenv-destroyenvironmentblock">Docs</a>
     */
    boolean DestroyEnvironmentBlock(Pointer lpEnvironment);

    /**
     * typedef struct _PROFILEINFOA { DWORD dwSize; DWORD dwFlags; MIDL_STRING LPSTR lpUserName; MIDL_STRING LPSTR
     * lpProfilePath; MIDL_STRING LPSTR lpDefaultPath; MIDL_STRING LPSTR lpServerName; MIDL_STRING LPSTR lpPolicyPath;
     * #if ... ULONG_PTR hProfile; #else HANDLE hProfile; #endif } PROFILEINFOA, *LPPROFILEINFOA;
     */
