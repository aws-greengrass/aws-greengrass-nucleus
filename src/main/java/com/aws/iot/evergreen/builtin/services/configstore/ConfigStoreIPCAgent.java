...
public class ConfigStoreIPCAgent implements InjectionActions {
    ...
    private final ChildChanged onConfigChange = (whatHappened, node) -> {
        ...
        // The path should have at least 5 items: null (root), services, serviceName, parameters/runtime, <someKey>
        if (nodePath.size() < 5) {
            return;
        }
        ...
        if (nodePath.get(configIndex).equals(PARAMETERS_CONFIG_KEY)) {
            subscribers.stream().map(ctx -> configUpdateListeners.get(ctx))
                    .forEach(c -> c.accept(nodePath.get(configIndex - 1)));
        } else if (nodePath.get(configIndex).equals(RUNTIME_STORE_NAMESPACE_TOPIC)) {
            ...
            subscribers.stream().filter(ctx -> !ctx.getServiceName().equals(serviceName))
                    .map(ctx -> configUpdateListeners.get(ctx)).forEach(c -> c.accept(nodePath.get(configIndex - 1)));
        } else {
            return;
        }
