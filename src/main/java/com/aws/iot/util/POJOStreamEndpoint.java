...
import static com.aws.iot.util.POJOUtil.*;
...
public class POJOStreamEndpoint implements Closeable, Flushable {
    ...
    public POJOStreamEndpoint(InputStream in, OutputStream out,
            Closeable cl, Dispatcher dispatcher) {
    ...
    public synchronized <T> T getSideCar(Class<T> claz) {
        if (claz.isInstance(sideCar)) return (T) sideCar;
        if (sideCar == null) try {
                ...
                public void run() {
                    ...
                    while (in0!=null && reader.next()!=null) {
                        try {
                            ...
                            System.out.println("Endpoint read error: "
                                    + getName() + " " + t);
                            break;
                    ...
                    in0 = null;
                    Utils.close(reader);
                    dispatcher.dispatch(POJOStreamEndpoint.this, POJOUtil.map("op",
                            "disconnected"));
                    if (actives != null) actives.remove(POJOStreamEndpoint.this);
                    close();
                }
            };
    ...
    public synchronized void sendObject(Callback callback, Map map) throws IOException {
        ...
        if(closedBy==null) try {
            if (callback != null) {
                callbacks.put(key, callback);
                map.put("syncid", key);
                // TODO: implement timeouts
            }
            if (verbose) System.out.println("Wrote " + getName()
                                   + " " + deepToString(map));
            if(out!=null)
        ...
        if(callback!=null)
            callback.callback(map("error", closedBy));
        throw closedBy;
    }
    ...
    public static class Server {
        ...
        public void stop() {
            Utils.close(ss);
            ss = null;
            if (verbose) System.out.println("JsonEndpoint server stopping");
        }
        ...
        private Server start() throws IOException {
            ...
            new Thread() {
                ...
                public void run() {
                    ...
                    while (ss != null)
                        try {
                            new POJOStreamEndpoint(ss.accept(), dispatcher).start(
                                    actives);
                            errs = 0;
     ...
     */
    public static POJOStreamEndpoint startConnection(String host, int port,
            Dispatcher dispatcher, Closeable cl) throws IOException {
        Socket s = new Socket(host, port);
        if(verbose) System.out.println("Connecting to socket "+s);
        return new POJOStreamEndpoint(s, dispatcher, cl).start(
                null);
    }
    public static POJOStreamEndpoint startConnection(String host,
            Dispatcher dispatcher, Closeable cl) throws IOException {
        return startConnection(host, JsonServerPort, dispatcher, cl).start(null);
    }
    public static POJOStreamEndpoint startConnection(String host,
            Dispatcher dispatcher) throws IOException {
        return startConnection(host, JsonServerPort, dispatcher, null).start(
                null);
    }
    ...
    public static class Dispatcher {
        ...
        public Dispatcher(Dispatcher p) { parent = p; }
        ...
        public Dispatcher add(Operator c) {
            ...
            if(on==null) throw new IllegalArgumentException(c.getClass().getSimpleName()+"must be annotated with OperatorNamed");
            return add(on.value(),c);
        }
        public Dispatcher remove(String name) {
            if(parent!=null) parent = parent.remove(name);
            ops.remove(name);
        ...
        public void dispatch(POJOStreamEndpoint e, Object o) {
            ...
            if (doAsync == null) doAsync = Boolean.TRUE;
            if (doAsync == Boolean.FALSE) dispatch0(e, o);
            else exec.submit(() -> dispatch0(e, o));
        }
            ...
            try {
                ...
                if (f == null) 
                    if(parent==null) throw new IllegalArgumentException(
                            "Undefinded op '" + opo + "'");
                    else dispatch0(e, parent);
                else {
                    Object ret = f.doIt(e, (Map) o);
                    if (ret instanceof Throwable) throw (Throwable) ret;
                    if (syncid != null)
                        e.sendMap("op", "reply",
                                "forid", syncid,
                                "value", deepToString(ret));
                }
            } catch (Throwable ex) {
                System.out.println("ERROR " + e.getName() + " " + ex.toString() + " " +
                                   deepToString(o));
                try {
                    if (syncid == null)
                        e.sendMap("op", "error",
                                "error", ex.toString(),
                                "json", o);
                    else e.sendMap("op", "reply",
                                "forid", syncid,
                                "json", o,
                                "error", ex.toString());
                } catch (IOException ex1) {
                    System.out.println("Error while reporting error: "
                                       + e.getName() + " "
                                       + ex.toString() +
                                       "\n\t" + deepToString(o));
        ...
        }
        {
            ...
            add("connected", (e, o) -> {
                System.out.println("Connected to " + e.getName()
                                   + " " + POJOUtil.jsget(o, "socket"));
                return null;
            });
    ...
    private static Object doReply(POJOStreamEndpoint e, Map o) {
        ...
        System.out.println("Handling reply to " + e.getName()
                                   + " " + id);
        if (id == null)
            if (POJOUtil.isError(o)) try {
                e.dispatcher.uncaughtError.doIt(e, o);
            } catch (Throwable ex) {
                ex.printStackTrace(System.out);
            }
            else System.out.println("Missing forid in " + e.getName()
                                   + " " + deepToString(o));
        else {
            Callback c = e.callbacks.remove(id);
            System.out.println("Starting callback " + e.getName()
                                   + " " + c);
            if (c == null)
                System.out.println("Missing callback for " + e.getName()
                                   + " " + deepToString(o));
            else try {
    ...
    private static final ExecutorService exec
            = new ThreadPoolExecutor(2, 15, 30, SECONDS,
    ...
    static {
        ...
        if(!Exec.isWindows)
            new Thread() {
                { setPriority(MIN_PRIORITY); setName("invoke sysctl"); }
                @Override public void run() {
                    System.out.println(Exec.cmd("sudo", "sysctl", "-w",
