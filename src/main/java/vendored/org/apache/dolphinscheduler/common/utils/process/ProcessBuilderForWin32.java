 ...
 *
 * <p>
 * Each {@code ProcessBuilderForWindows} instance manages a collection of process attributes. The {@link #start()}
 * method creates a new {@link Process} instance with those attributes. The {@link #start()} method can be invoked
 ...
 * <ul>
 *
 * <li>a <i>command</i>, a list of strings which signifies the external program file to be invoked and its arguments, if
 * any. Which string lists represent a valid operating system command is system-dependent. For example, it is common for
 * each conceptual argument to be an element in this list, but there are operating systems where programs are expected
 * to tokenize command line strings themselves - on such a system a Java implementation might require commands to
 * contain exactly two elements.
 *
 * <li>an <i>environment</i>, which is a system-dependent mapping from <i>variables</i> to <i>values</i>. The initial
 * value is a copy of the environment of the current process (see {@link System#getenv()}).
 *
 * <li>a <i>working directory</i>. The default value is the current working directory of the current process, usually
 * the directory named by the system property {@code user.dir}.
 *
 * <li><a name="redirect-input">a source of <i>standard input</i></a>. By default, the subprocess reads input from a
 * pipe. Java code can access this pipe via the output stream returned by {@link Process#getOutputStream()}. However,
 * standard input may be redirected to another source using {@link #redirectInput(ProcessBuilderForWin32.Redirect)
 * redirectInput}. In this case, {@link Process#getOutputStream()} will return a <i>null output stream</i>, for which:
 *
 * <ul>
 ...
 * </ul>
 *
 * <li><a name="redirect-output">a destination for <i>standard output</i> and <i>standard error</i></a>. By default, the
 * subprocess writes standard output and standard error to pipes. Java code can access these pipes via the input streams
 * returned by {@link Process#getInputStream()} and {@link Process#getErrorStream()}. However, standard output and
 * standard error may be redirected to other destinations using {@link #redirectOutput(ProcessBuilderForWin32.Redirect)
 * redirectOutput} and {@link #redirectError(ProcessBuilderForWin32.Redirect) redirectError}. In this case,
 * {@link Process#getInputStream()} and/or {@link Process#getErrorStream()} will return a <i>null input stream</i>, for
 * which:
 *
 * <ul>
 ...
 * </ul>
 *
 * <li>a <i>redirectErrorStream</i> property. Initially, this property is {@code false}, meaning that the standard
 * output and error output of a subprocess are sent to two separate streams, which can be accessed using the
 * {@link Process#getInputStream()} and {@link Process#getErrorStream()} methods.
 *
 * <p>
 * If the value is set to {@code true}, then:
 *
 * <ul>
 * <li>standard error is merged with the standard output and always sent to the same destination (this makes it easier
 * to correlate error messages with the corresponding output)
 * <li>the common destination of standard error and standard output can be redirected using
 * {@link #redirectOutput(ProcessBuilderForWin32.Redirect) redirectOutput}
 * <li>any redirection set by the {@link #redirectError(ProcessBuilderForWin32.Redirect) redirectError} method is
 * ignored when creating a subprocess
 * <li>the stream returned from {@link Process#getErrorStream()} will always be a <a href="#redirect-output">null input
 * stream</a>
 * </ul>
 *
 * </ul>
 ...
 *
 * <p>
 * Most error checking is performed by the {@link #start()} method. It is possible to modify the state of an object so
 * that {@link #start()} will fail. For example, setting the command attribute to an empty list will not throw an
 * exception unless {@link #start()} is invoked.
 *
 * <p>
 * <strong>Note that this class is not synchronized.</strong> If multiple threads access a
 * {@code ProcessBuilderForWindows} instance concurrently, and at least one of the threads modifies one of the
 ...
 *
 * <pre> {@code
 * ProcessBuilderForWindows pb = new ProcessBuilderForWindows("myCommand", "myArg1", "myArg2");
 ...
 *
 * <p>
 * To start a process with an explicit set of environment variables, first call {@link Map#clear() Map.clear()} before
...
public class ProcessBuilderForWin32 {
     ...
     * constructor that sets the process builder's command to a string list containing the same strings as the
     * {@code command} array, in the same order. It is not checked whether {@code command} corresponds to a valid
     ...
     *
     * <p>
     * The returned object may be modified using ordinary {@link Map Map} operations. These modifications will be
     * visible to subprocesses started via the {@link #start()} method. Two {@code ProcessBuilderForWindows} instances
     * always contain independent process environments, so changes to the returned map will never be reflected in any
     * other {@code ProcessBuilderForWindows} instance or the values returned by {@link System#getenv System.getenv}.
     *
     * <p>
     ...
     *
     * @throws SecurityException if a security manager exists and its {@link SecurityManager#checkPermission
    ...
    ProcessBuilderForWin32 environment(String[] envp) {
        ...
        if (envp != null) {
            ...
            for (String envstring : envp) {
                ...
                int eqlsign = envstring.indexOf('=', ProcessEnvironmentForWin32.MIN_NAME_LENGTH);
     ...
     * <li>a redirection to read from a file, created by an invocation of {@link ProcessBuilderForWin32.Redirect#from
     * Redirect.from(File)}
     * <li>a redirection to write to a file, created by an invocation of {@link ProcessBuilderForWin32.Redirect#to
     * Redirect.to(File)}
     ...
     *
     * <p>
     * Each of the above categories has an associated unique {@link ProcessBuilderForWin32.Redirect.Type Type}.
     *
     * @since 1.7
     */
    public abstract static class Redirect {
        ...
        public enum Type {
            ...
            /**
             * The type of redirects returned from {@link ProcessBuilderForWin32.Redirect#from Redirect.from(File)}.
             */
            READ,

            /**
             * The type of redirects returned from {@link ProcessBuilderForWin32.Redirect#to Redirect.to(File)}.
             */
            WRITE,

            /**
             * The type of redirects returned from {@link ProcessBuilderForWin32.Redirect#appendTo
        ...
        /**
         * Indicates that subprocess I/O source or destination will be the same as those of the current process. This is
         * the normal behavior of most operating system command interpreters (shells).
         *
         * <p>
         * It will always be true that
         * 
        ...
        /**
         * Returns the {@link File} source or destination associated with this redirect, or {@code null} if there is no
         * such file.
         *
         * @return the file associated with this redirect, or {@code null} if there is no such file
         */
        public File file() {
            return null;
        }
        ...
        /**
         * Returns a redirect to write to the specified file. If the specified file exists when the subprocess is
         * started, its previous contents will be discarded.
         *
         * <p>
         * It will always be true that
         * 
        ...
        /**
         * Returns a redirect to append to the specified file. Each write operation first advances the position to the
         * end of the file and then writes the requested data. Whether the advancement of the position and the writing
         * of the data are done in a single atomic operation is system-dependent and therefore unspecified.
         *
         * <p>
         * It will always be true that
         * 
        ...
        /**
         * Compares the specified object with this {@code Redirect} for equality. Returns {@code true} if and only if
         * the two objects are identical or both objects are {@code Redirect} instances of the same type associated with
         * non-null equal {@code File} instances.
         */
        @Override
     ...
     *
     * Subprocesses subsequently started by this object's {@link #start()} method obtain their standard input from this
     * source.
     *
     * <p>
     * If the source is {@link ProcessBuilderForWin32.Redirect#PIPE Redirect.PIPE} (the initial value), then the
     * standard input of a subprocess can be written to using the output stream returned by
     * {@link Process#getOutputStream()}. If the source is set to any other value, then
     * {@link Process#getOutputStream()} will return a <a href="#redirect-input">null output stream</a>.
     *
     ...
     *
     * <p>
     * If the destination is {@link ProcessBuilderForWin32.Redirect#PIPE Redirect.PIPE} (the initial value), then the
     * standard output of a subprocess can be read using the input stream returned by {@link Process#getInputStream()}.
     * If the destination is set to any other value, then {@link Process#getInputStream()} will return a
     * <a href="#redirect-output">null input stream</a>.
     *
     * @param destination the new standard output destination
     * @return this process builder
     * @throws IllegalArgumentException if the redirect does not correspond to a valid destination of data, that is, has
     *         type {@link ProcessBuilderForWin32.Redirect.Type#READ READ}
     * @since 1.7
     */
     ...
     * <p>
     * If the destination is {@link ProcessBuilderForWin32.Redirect#PIPE Redirect.PIPE} (the initial value), then the
     * error output of a subprocess can be read using the input stream returned by {@link Process#getErrorStream()}. If
     * the destination is set to any other value, then {@link Process#getErrorStream()} will return a
     * <a href="#redirect-output">null input stream</a>.
     *
     * <p>
     * If the {@link #redirectErrorStream redirectErrorStream} attribute has been set {@code true}, then the redirection
     * set by this method has no effect.
     *
     ...
     *
     * <p>
     * This is a convenience method. An invocation of the form {@code redirectInput(file)} behaves in exactly the same
     * way as the invocation {@link #redirectInput(ProcessBuilderForWin32.Redirect) redirectInput}
     ...
     *
     * <p>
     * This is a convenience method. An invocation of the form {@code redirectOutput(file)} behaves in exactly the same
     * way as the invocation {@link #redirectOutput(ProcessBuilderForWin32.Redirect) redirectOutput}
     ...
     *
     * <p>
     * This is a convenience method. An invocation of the form {@code redirectError(file)} behaves in exactly the same
     * way as the invocation {@link #redirectError(ProcessBuilderForWin32.Redirect) redirectError}
     * {@code (Redirect.to(file))}.
     *
     ...
     *
     * Subprocesses subsequently started by this object's {@link #start()} method obtain their standard input from this
     * source. The initial value is {@link ProcessBuilderForWin32.Redirect#PIPE Redirect.PIPE}.
     *
     ...
     *
     * Subprocesses subsequently started by this object's {@link #start()} method redirect their standard output to this
     * destination. The initial value is {@link ProcessBuilderForWin32.Redirect#PIPE Redirect.PIPE}.
     *
     ...
     *
     * Subprocesses subsequently started by this object's {@link #start()} method redirect their standard error to this
     * destination. The initial value is {@link ProcessBuilderForWin32.Redirect#PIPE Redirect.PIPE}.
     *
     ...
     * }</pre>
     *
     * This gives behavior equivalent to most operating system command interpreters, or the standard C library function
     * {@code system()}.
     *
     * @return this process builder
     * @since 1.7
     ...
     *
     *         <li>the standard input to the subprocess was {@linkplain #redirectInput redirected from a file} and the
     *         security manager's {@link SecurityManager#checkRead checkRead} method denies read access to the file, or
     *
     *         <li>the standard output or standard error of the subprocess was {@linkplain #redirectOutput redirected to
     *         a file} and the security manager's {@link SecurityManager#checkWrite checkWrite} method denies write
     *         access to the file
     *
     *         </ul>
     *
    ...
    public Process start() throws IOException {
        ...
        try {
            return ProcessImplForWin32.start(username, password, cmdarray, defaultEnvironment, environment, dir,
                    redirects, redirectErrorStream, processCreationFlags);
