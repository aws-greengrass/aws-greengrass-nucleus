...
import software.amazon.eventstream.iot.model.AccessDeniedException;
...
public class EventStreamRPCConnection {
    ...
    private final Lock connectionEstablishmentLock;
    ...
    public EventStreamRPCConnection(final EventStreamRPCConnectionConfig config) {
        this.config = config;
        this.connectionEstablishmentLock = new ReentrantLock();
        this.connection = null;
        this.pendingConnection = null;
    }

    public void connect(final LifecycleHandler lifecycleHandler) {
        if (!connectionEstablishmentLock.tryLock()) {
                    ...
                    protected void onProtocolMessage(List<Header> headers, byte[] payload, MessageType messageType, int messageFlags) {
                        if (MessageType.ConnectAck.equals(messageType)) {
                            try {
                                ...
                                connectionEstablishmentLock.unlock();
                            }
    ...
    public void disconnect() {
        //TODO: probable race condition between establishing a connection and disconnecting one?
        if (connection != null) {
            connection.closeConnection(0);
        }
    }
