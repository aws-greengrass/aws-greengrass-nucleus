...
public interface OperationContinuationHandlerFactory {
    Function<OperationContinuationHandlerContext, ? extends ServerConnectionContinuationHandler> getOperationHandler(
    ...
    /**
     * this may not be a good use of a default method impl as implementers can override it also
     * InvalidServiceConfigurationException is a needed exception to be thrown from IpcServer
     */
    default void validateAllOperationsSet() {
        if (!getAllOperations().stream().allMatch(op -> hasHandlerForOperation(op))) {
            String unmappedOperations = getAllOperations().stream()
                    .filter(op -> !hasHandlerForOperation(op))
